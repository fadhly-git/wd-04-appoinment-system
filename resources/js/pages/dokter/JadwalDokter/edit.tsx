...
interface JadwalDokterPageProps extends SharedData {
    jadwals: JadwalPeriksa;
}

export default function CreateObat() {
    const { jadwals, auth } = usePage<JadwalDokterPageProps>().props;
    const { data, setData, patch, errors, processing } = useForm<JadwalPeriksa>({
        id: jadwals?.id || 0, // Assuming you want to initialize id as well
        id_dokter: auth.user?.id || 0,
        hari: jadwals?.hari || '',
        jam_mulai: jadwals?.jam_mulai || '',
        jam_selesai: jadwals?.jam_selesai || '',
        status: jadwals?.status || 0, // Default status is active
    });

    useEffect(() => {
        setData({
            id: jadwals.id || 0,
            id_dokter: jadwals.id_dokter || 0,
            hari: jadwals.hari || '',
            jam_mulai: jadwals.jam_mulai || '',
            jam_selesai: jadwals.jam_selesai || '',
            status: jadwals.status || 1,
        });
    }, [jadwals]);
    if (!auth.user) {
    ...
    const handleSubmit: FormEventHandler = (e) => {
        e.preventDefault();
        patch(route('dokter.jadwal.update', { id: data.id }), {
            onSuccess: () => {
                ...
                setData({
                    id: 0, // Reset id to 0 for new entry
                    id_dokter: auth.user?.id || 0,
                    hari: '',
                    jam_mulai: '',
                    jam_selesai: '',
            ...
            onError: (e) => {
                ...
                console.error('Error edit jadwal:', e);
            },
        });
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Edit Jadwal Periksa" />
            <div className="flex flex-1 flex-col gap-4 rounded-xl p-4">
                <div className="border-sidebar-border/70 dark:border-sidebar-border flex flex-1 flex-row items-center justify-between rounded-xl border p-4">
                    <div className="flex">
                        <h1 className="text-lg font-black">Edit Data Jadwal</h1>
                    </div>
                    <div className="flex">
            ...
            <div className="flex flex-col gap-4 rounded-xl p-4">
                <div className="border-sidebar-border/70 dark:border-sidebar-border items-center rounded-xl border p-4">
                    <h1 className="mb-6 text-xl font-black">Form Edit Jadwal</h1>
                    <form className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <Select value={data.status.toString()} onValueChange={(value) => setData('status', parseInt(value))} required>
                                <SelectTrigger className="w-full">
